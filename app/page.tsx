...
type CompetitionData = { records: CompetitionRecord[]; editions: { editionNo: number; edition: string; schoolRecords: number; schools: number; counties: number; awardSchools: number; teams: number }[]; schools: { county: string; school: string; editions: number[]; awardEditions: number[] }[]; meta: { note: string; updated: string } };
...
function CompetitionPage({ data }: { data: CompetitionData | null }) {
  ...
  const totalTeams = data?.editions.reduce((total, item) => total + item.teams, 0) ?? 0;
  ...
  return <main className="competition-page">
        <section className="competition-hero"><div><p className="eyebrow"><Sparkles size={20} /> 廣達游智盃 · 參與足跡</p><h1>每一次參加，<br /><em>都是一次把想法做出來的挑戰。</em></h1></div><div className="competition-hero-card"><span>COMPETITION JOURNEY</span><strong>1—7 <b>屆</b></strong><small>廣達游智盃參與紀錄</small></div></section>
    <section className="competition-content"><div className="section-heading split-heading"><div><p className="section-kicker">QUANTA CREATIVE PROGRAMMING CUP</p><h2>看見學校持續參與的足跡</h2></div></div>
      <div className="metrics-section competition-metrics"><MetricCard value={data?.editions.length ?? 0} label="辦理屆次" note="目前整理至第七屆" /><MetricCard value={totalSchools} label="不重複參與學校" note="跨屆只計一次" /><MetricCard value={records.length} label="學校參與校次" note="一校一屆一筆" /><MetricCard value={new Set(records.map((r) => r.county)).size} label="參與地區" note="含臺灣縣市與海外地區" /><MetricCard value={totalTeams} label="歷屆參與隊伍數" note="第 1–7 屆隊伍數合計" /></div>
      ...
      <p className="competition-footnote">廣達游智盃資料更新：{data?.meta.updated}。</p>
    </section>
  </main>;
}
    ...
    <main>
      ...
      <section className="hero" id="top">
        ...
        <div className="hero-copy">
          ...
          <h1>用科技打開想像，<br />培養孩子與未來世界溝通的能力。</h1>
          <p className="hero-lead">廣達《游於智》計畫從教師增能、程式與機電整合課程，到縣市合作與教具推廣，陪伴孩子從生活問題出發、動手實作，培養運用科技思考、表達與解決問題的能力。</p>
